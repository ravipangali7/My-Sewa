...
def data_pack_history(request):
    ...
    qs = DataPackTransaction.objects.filter(user=request.user).order_by('-created_at')
    ...
    qs = apply_search(
        qs,
        params['search'],
        ['mobile_number', 'package_name', 'merchant_txn_id', 'reference_id', 'operator'],
    )
    stats = txn_stats(qs)
    data = DataPackTransactionSerializer(qs, many=True).data
    if params['format'] == 'csv':
        ...
        return csv_response(
            ...
            rows,
            [
                'id', 'created_at', 'operator', 'mobile_number', 'package',
