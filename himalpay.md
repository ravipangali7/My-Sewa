...
### 6. Get Reseller Statement

Retrieve your account's own transaction/ledger statement. This endpoint is not paginated - every matching entry is returned in a single response.

- URL:`/statement/reseller-statement`
- Method:`GET`
- Query Parameters:
- - `transaction_id`(string, optional): Filter by transaction UUID (the`transaction_id` value returned from a payment/load response).
- `from_date`(string, optional): Start date, format`YYYY-MM-DD`.
- `to_date`(string, optional): End date, format`YYYY-MM-DD`.
- Response Body:
- - `data`(array): List of statement/ledger entries, newest first (see fields below).

[!IMPORTANT]

- Since there is no pagination,`from_date`/`to_date` together must not span more than 2 months. Requesting a wider range returns a`400` error.
- If neither`from_date` nor`to_date` is supplied (and no`transaction_id`), the last 2 months are returned by default.
- If only`transaction_id` is supplied, the lookup is not date-bound - you can retrieve a transaction regardless of its age.
- Errors from this endpoint return a simple`{ "error": " " }` body (no`error_code`/`error_type`).

`data` item fields:

- `direction`(string):`debit` or`credit`.
- `amount`(int): Entry amount, in paisa.
- `balance_before`/`balance_after`(int): Your main wallet balance before/after this entry, in paisa.
- `bonus_balance_before`/`bonus_balance_after`(int): Your bonus wallet balance before/after this entry, in paisa.
- `is_refund`/`is_cashback`/`is_charge`(bool): Entry classification flags.
- `reference_id`(string, optional): External operator reference, if available.
- `created_at`(string): Entry timestamp.
- `transaction_uuid`(string): Transaction UUID - same value returned as`transaction_id` from payment/load responses.
- `status`(string, optional): Transaction status (`SUCCESS`,`FAILED`,`UNKNOWN`).
- `wallet_service_name`(string): Wallet service name.
- `wallet_service_logo_url`(string, optional): Wallet service logo URL.
- `transaction_cashback`/`transaction_charge`(int): Total cashback/charge on the parent transaction, in paisa.

#### Get Reseller Statement Example

Request (date range, max 2 months):

```
curl --request GET \
  --url 'https://uatapi.himalpay.com.np/api/v1/statement/reseller-statement?from_date=2026-07-01&to_date=2026-08-06' \
  --header 'x-api-key: YOUR_API_KEY'
  ...
  "data": [
    {
      ...
      "is_charge": false,
      "reference_id": "REF123",
      "created_at": "2026-08-01T10:15:32+05:45",
      "transaction_uuid": "TXN-789-XYZ",
      "status": "SUCCESS",
      "wallet_service_id": 1,
...
Request (lookup by transaction ID - not date bound):

```
curl --request GET \
  --url 'https://uatapi.himalpay.com.np/api/v1/statement/reseller-statement?transaction_id=TXN-789-XYZ' \
  --header 'x-api-key: YOUR_API_KEY'

```

Error - date range exceeds 2 months:

```
{
  "error": "date range must not exceed 2 months"
}

```

### 7. Get Reseller Balance

Retrieve your account's current wallet balance.

- URL:`/wallet/reseller-balance`
- Method:`GET`
- Response Body:
- - `id`(int): Internal wallet ID.
- `user_id`(int): Your internal user/account ID.
- `balance`(int): Main wallet balance, in paisa.
- `bonus_balance`(int): Bonus wallet balance, in paisa.
- `balance_in_rupees`(float): Main wallet balance, in rupees.
- `bonus_balance_in_rupees`(float): Bonus wallet balance, in rupees.
- `total_balance_in_rupees`(float): Main + bonus balance, in rupees.
- `created_at`/`updated_at`(string): Wallet timestamps.

#### Get Reseller Balance Example

Request:

```
curl --request GET \
  --url https://uatapi.himalpay.com.np/api/v1/wallet/reseller-balance \
  --header 'x-api-key: YOUR_API_KEY'
...
```
{
  ...
  "balance": 5000000,
  "bonus_balance": 0,
  "balance_in_rupees": 50000,
  "bonus_balance_in_rupees": 0,
  "total_balance_in_rupees": 50000,
  "created_at": "2026-01-15T09:12:03+05:45",
  "updated_at": "2026-08-06T11:40:21+05:45"
}
...
```
curl --request GET \
  --url https://uatapi.himalpay.com.np/api/v1/my-reseller-services \
  --header 'x-api-key: YOUR_API_KEY'
...
> [!IMPORTANT] MySewa handling: When a remittance is already paid/received (or locked), HimalPay often still returns outer `status: SUCCESS` with empty/zero `payout_amt` and the real reason in `data.vendor_state` / `vendor_status` (e.g. "Already Received", "Amount is locked"). Do **not** surface a generic "missing amount 0" / Invalid amount error in that case - return the vendor message, and map already-received/paid phrases to `error: Already received`.
