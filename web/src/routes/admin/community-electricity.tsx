...
export const Route = createFileRoute("/admin/community-electricity")({
  head: () => ({
    meta: [
      { title: "Community Power Ledger — MySewa Admin" },
      {
        name: "description",
        content:
          "Community electricity bill ledger with providers, counters, charges, cashback and provider responses.",
      },
      { property: "og:title", content: "Community Power Ledger — MySewa Admin" },
      {
        property: "og:description",
        content: "Community electricity transaction oversight for support teams.",
      },
    ],
  }),
  component: CommunityElectricityPage,
});
...
function CommunityElectricityPage() {
  ...
  const billsQuery = useQuery({
    queryKey: ["admin", "community-electricity", debounced, statusTab],
    queryFn: () =>
      apiClient.adminCommunityElectricity({
        ...debounced,
        status: statusTab,
      }),
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: TxnStatus }) =>
      apiClient.adminUpdateCommunityElectricityStatus(id, status),
    onSuccess: (_res, vars) => {
      toast.success(`Community power #${vars.id} marked ${vars.status}`);
      queryClient.invalidateQueries({ queryKey: ["admin", "community-electricity"] });
  ...
  const openDetail = (id: number) => {
    navigate({
      to: "/admin/community-electricity/$communityElectricityId",
      params: { communityElectricityId: String(id) },
    });
  };
  ...
  const statusSelect = (id: number, status: TxnStatus) => (
    ...
    >
      <SelectTrigger
        ...
        aria-label={`Status for community power bill ${id}`}
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
  ...
  return (
    <AdminShell title="Community Power" description="Community electricity bill ledger">
      {billsQuery.isLoading && (
        <p className="mb-4 text-sm text-muted-foreground">Loading community power bills…</p>
      )}
      {billsQuery.isError && (
        <p className="mb-4 text-sm text-destructive">
          {billsQuery.error instanceof ApiError
            ? billsQuery.error.message
            : "Could not load community power bills."}
        </p>
      )}
      ...
      <div className="space-y-4">
        ...
        <ListPageToolbar
          ...
          onExport={async () => {
            ...
            try {
              await downloadCsvExport(
                "/api/admin/community-electricity/",
          ...
          exporting={exporting}
          searchPlaceholder="Search phone, provider, customer ref, txn ID…"
        ...
        <AdminDataList
          ...
          empty={
            <AdminEmptyState>
              No {emptyLabel ? `${emptyLabel} ` : ""}community power bills.
            </AdminEmptyState>
          }
          table={
            <Table>
              ...
              <TableBody>
                {items.map((t, index) => (
                  ...
                  >
                    ...
                    <TableCell className="text-sm font-medium">{t.platform_name}</TableCell>
                    <TableCell className="text-sm">{t.customer_ref}</TableCell>
                    <TableCell className="max-w-40 truncate text-sm" title={t.counter_code}>
                      {t.counter_code || "—"}
                    </TableCell>
                ...
                <AdminMobileCard key={t.id} onClick={() => openDetail(t.id)}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-start gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-sm font-semibold">{t.platform_name}</p>
                  ...
                  <AdminMobileMeta
                    items={[
                      ...
                      { label: "Counter", value: t.counter_code || "—" },
