...
export const Route = createFileRoute("/admin/internet")({
  head: () => ({
    meta: [
      { title: "Internet Ledger — MySewa Admin" },
      {
        name: "description",
        content:
          "ISP internet bill ledger with customer IDs, packages, charges, cashback and provider responses.",
      },
      { property: "og:title", content: "Internet Ledger — MySewa Admin" },
      {
        property: "og:description",
        content: "Internet bill transaction oversight for support teams.",
      },
    ],
  }),
...
function InternetPage() {
  ...
  const billsQuery = useQuery({
    queryKey: ["admin", "internet-bills", debounced, statusTab],
    queryFn: () =>
      apiClient.adminInternetBills({
        ...debounced,
        status: statusTab,
      }),
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: TxnStatus }) =>
      apiClient.adminUpdateInternetBillStatus(id, status),
    onSuccess: (_res, vars) => {
      toast.success(`Internet bill #${vars.id} marked ${vars.status}`);
      queryClient.invalidateQueries({ queryKey: ["admin", "internet-bills"] });
  ...
  const openDetail = (id: number) => {
    navigate({ to: "/admin/internet/$internetId", params: { internetId: String(id) } });
  };
  ...
  const statusSelect = (id: number, status: TxnStatus) => (
    ...
    >
      <SelectTrigger
        ...
        aria-label={`Status for internet bill ${id}`}
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
  ...
  return (
    <AdminShell title="Internet" description="ISP bill payment ledger">
      {billsQuery.isLoading && (
        <p className="mb-4 text-sm text-muted-foreground">Loading internet bills…</p>
      )}
      {billsQuery.isError && (
        <p className="mb-4 text-sm text-destructive">
          {billsQuery.error instanceof ApiError
            ? billsQuery.error.message
            : "Could not load internet bills."}
        </p>
      )}
      ...
      <div className="space-y-4">
        ...
        <ListPageToolbar
          ...
          exporting={exporting}
          searchPlaceholder="Search phone, ISP, customer ID, txn ID…"
        ...
        <AdminDataList
          ...
          empty={
            <AdminEmptyState>
              No {emptyLabel ? `${emptyLabel} ` : ""}internet bills.
            </AdminEmptyState>
          }
          table={
            <Table>
              ...
              <TableBody>
                {items.map((t, index) => (
                  ...
                  >
                    ...
                    <TableCell className="text-sm font-medium">{t.isp_name}</TableCell>
                    <TableCell className="text-sm">{t.customer_id}</TableCell>
                ...
                <AdminMobileCard key={t.id} onClick={() => openDetail(t.id)}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-start gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-sm font-semibold">{t.isp_name}</p>
                  ...
                  <AdminMobileMeta
                    items={[
                      ...
                      { label: "Package", value: t.package_name || "—" },
