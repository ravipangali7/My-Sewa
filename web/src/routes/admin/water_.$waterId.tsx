...
export const Route = createFileRoute("/admin/water_/$waterId")({
  head: () => ({
    meta: [
      { title: "Khanepani Statement — MySewa Admin" },
      {
        name: "description",
        content:
          "View full KUKL water bill statement including connection details, charges, cashback, wallet debit, and provider response.",
      },
      { property: "og:title", content: "Khanepani Statement — MySewa Admin" },
    ],
  }),
  component: WaterDetailPage,
});
...
function WaterDetailPage() {
  const { waterId } = Route.useParams();
  const id = Number(waterId);

  const detailQuery = useQuery({
    queryKey: ["admin", "water-bills", id],
    queryFn: () => apiClient.adminGetWaterBill(id),
  ...
  return (
    <AdminShell
      title={t ? `Khanepani #${t.id}` : "Khanepani"}
      description={
        t
    ...
    >
      <div className="mb-5">
        <BackButton to="/admin/water" label="Back to Khanepani" />
      </div>
          ...
          <article className="min-w-0 overflow-x-clip rounded-2xl border border-border bg-surface shadow-card">
              ...
              <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="min-w-0 rounded-xl border border-brand/20 bg-gradient-to-br from-brand-soft/70 via-surface to-surface px-4 py-5 sm:px-5">
                  ...
                  <p className="mt-1.5 break-words text-sm text-muted-foreground">
                    {t.connection_no} · {t.customer_code}
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-0 md:grid-cols-2">
              ...
              <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
                ...
                <dl>
                  <StatementRow label="Connection">{t.connection_no}</StatementRow>
                  <StatementRow label="Customer code">{t.customer_code}</StatementRow>
                  <StatementRow label="Counter">{t.counter || "—"}</StatementRow>
                  <StatementRow label="Customer name">{t.customer_name || "—"}</StatementRow>
                  <StatementRow label="Payment type">{t.payment_type || "—"}</StatementRow>
                </dl>
              </section>
            </div>
            ...
            <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
              ...
              <dl>
                <StatementRow label="Type">Khanepani · KUKL</StatementRow>
            ...
            <footer className="border-t border-border bg-muted/40 px-4 py-3 text-xs text-muted-foreground sm:px-6 md:px-8">
              This statement summarizes the Khanepani (KUKL) water bill payment recorded in MySewa.
              Wallet debit equals bill amount plus charge minus cashback.
            </footer>
          </article>
        </div>
      )}
