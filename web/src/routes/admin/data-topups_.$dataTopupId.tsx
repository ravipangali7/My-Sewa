...
export const Route = createFileRoute("/admin/data-topups_/$dataTopupId")({
  head: () => ({
    meta: [
      { title: "Data Top-Up Statement — MySewa Admin" },
      {
        name: "description",
        content:
          "View full data pack statement including package details, charges, cashback, wallet debit, and provider response.",
      },
      { property: "og:title", content: "Data Top-Up Statement — MySewa Admin" },
    ],
  }),
  component: DataTopupDetailPage,
});
...
function DataTopupDetailPage() {
  const { dataTopupId } = Route.useParams();
  const id = Number(dataTopupId);
  ...
  return (
    <AdminShell
      title={t ? `Data Top-Up #${t.id}` : "Data Top-Up"}
      description={
        t
          ? "Data pack statement"
    ...
    >
      <div className="mb-5">
        <BackButton to="/admin/data-topups" label="Back to data top-ups" />
      </div>
          ...
          <article className="min-w-0 overflow-x-clip rounded-2xl border border-border bg-surface shadow-card">
            ...
            <div className="grid grid-cols-1 gap-0 md:grid-cols-2">
              ...
              <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
                ...
                <dl>
                  <StatementRow label="Operator">{t.operator}</StatementRow>
                  <StatementRow label="Mobile">{t.mobile_number}</StatementRow>
                  <StatementRow label="Package">{t.package_name || "—"}</StatementRow>
                  <StatementRow label="Package ID">{t.package_id || "—"}</StatementRow>
                  <StatementRow label="Product code">{t.product_code || "—"}</StatementRow>
                </dl>
              </section>
            </div>
            ...
            <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
              ...
              <dl>
                <StatementRow label="Type">Data pack · {t.operator}</StatementRow>
            ...
            <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
              ...
              <dl>
                <StatementRow label="Package">{formatNPR(t.amount)}</StatementRow>
            ...
            <footer className="border-t border-border bg-muted/40 px-4 py-3 text-xs text-muted-foreground sm:px-6 md:px-8">
              This statement summarizes the data pack purchase recorded in MySewa. Wallet debit
              equals package amount plus charge minus cashback.
            </footer>
          </article>
        </div>
      )}
