...
export const Route = createFileRoute("/admin/internet_/$internetId")({
  head: () => ({
    meta: [
      { title: "Internet Statement — MySewa Admin" },
      {
        name: "description",
        content:
          "View full internet bill statement including ISP details, charges, cashback, wallet debit, and provider response.",
      },
      { property: "og:title", content: "Internet Statement — MySewa Admin" },
    ],
  }),
  component: InternetDetailPage,
});
...
function InternetDetailPage() {
  const { internetId } = Route.useParams();
  const id = Number(internetId);

  const detailQuery = useQuery({
    queryKey: ["admin", "internet-bills", id],
    queryFn: () => apiClient.adminGetInternetBill(id),
  ...
  return (
    <AdminShell
      title={t ? `Internet #${t.id}` : "Internet"}
      description={
        t
    ...
    >
      <div className="mb-5">
        <BackButton to="/admin/internet" label="Back to internet" />
      </div>
          ...
          <article className="min-w-0 overflow-x-clip rounded-2xl border border-border bg-surface shadow-card">
            ...
            <div className="grid grid-cols-1 gap-0 md:grid-cols-2">
              ...
              <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
                ...
                <dl>
                  <StatementRow label="ISP">{t.isp_name}</StatementRow>
                  <StatementRow label="ISP ID">{t.isp_id}</StatementRow>
                  <StatementRow label="Customer ID">{t.customer_id}</StatementRow>
                  <StatementRow label="Customer name">{t.customer_name || "—"}</StatementRow>
                  <StatementRow label="Package">{t.package_name || "—"}</StatementRow>
                </dl>
              </section>
            </div>
            ...
            <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
              ...
              <dl>
                <StatementRow label="Type">Internet bill · {t.isp_name}</StatementRow>
            ...
            <footer className="border-t border-border bg-muted/40 px-4 py-3 text-xs text-muted-foreground sm:px-6 md:px-8">
              This statement summarizes the internet bill payment recorded in MySewa. Wallet debit
              equals bill amount plus charge minus cashback.
            </footer>
          </article>
        </div>
      )}
