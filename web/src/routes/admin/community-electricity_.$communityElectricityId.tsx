...
export const Route = createFileRoute(
  ...
  head: () => ({
    meta: [
      { title: "Community Power Statement — MySewa Admin" },
      {
        name: "description",
        content:
          "View full community electricity bill statement including provider details, charges, cashback, wallet debit, and provider response.",
      },
      { property: "og:title", content: "Community Power Statement — MySewa Admin" },
    ],
  }),
  component: CommunityElectricityDetailPage,
});
...
function displayName(t: { first_name?: string; last_name?: string; phone: string }) {
...
function CommunityElectricityDetailPage() {
  const { communityElectricityId } = Route.useParams();
  const id = Number(communityElectricityId);
  ...
  const accountName = t ? displayName(t) : "";
  const providerJson = t ? formatProviderResponse(t.provider_response) : null;

  return (
    <AdminShell
      title={t ? `Community Power #${t.id}` : "Community Power"}
      description={
        t
          ? "Community electricity bill statement"
    ...
    >
      <div className="mb-5">
        <BackButton to="/admin/community-electricity" label="Back to Community Power" />
      </div>
      ...
      {detailQuery.isError && (
        <p className="text-sm text-muted-foreground">
          {detailQuery.error instanceof ApiError
            ...
            : "Community power bill not found."}
        </p>
      )}
          ...
          <article className="min-w-0 overflow-x-clip rounded-2xl border border-border bg-surface shadow-card">
              ...
              <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="min-w-0 rounded-xl border border-brand/20 bg-gradient-to-br from-brand-soft/70 via-surface to-surface px-4 py-5 sm:px-5">
                  ...
                  <p className="mt-1.5 break-words text-sm text-muted-foreground">
                    {t.platform_name} · {t.customer_ref}
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-0 md:grid-cols-2">
              ...
              <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
                ...
                <dl>
                  <StatementRow label="Provider">{t.platform_name}</StatementRow>
                  <StatementRow label="Platform ID">{t.platform_id}</StatementRow>
                  <StatementRow label="Customer ref">{t.customer_ref}</StatementRow>
                  <StatementRow label="Consumer ID">{t.consumer_id || "—"}</StatementRow>
                  <StatementRow label="Counter">{t.counter_code || "—"}</StatementRow>
                  <StatementRow label="Customer name">{t.customer_name || "—"}</StatementRow>
                  <StatementRow label="Service">{t.service_slug || "—"}</StatementRow>
                  <StatementRow label="Month">{t.month != null ? String(t.month) : "—"}</StatementRow>
                </dl>
              </section>
            </div>
            ...
            <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
              ...
              <dl>
                <StatementRow label="Type">Community Power · {t.platform_name}</StatementRow>
                ...
                <StatementRow label="Session">{t.session_id || "—"}</StatementRow>
            ...
            <section className="border-b border-border px-4 py-5 sm:px-6 md:px-8">
              ...
              <dl>
                <StatementRow label="Bill">{formatNPR(t.amount)}</StatementRow>
            ...
            <footer className="border-t border-border bg-muted/40 px-4 py-3 text-xs text-muted-foreground sm:px-6 md:px-8">
              This statement summarizes the community electricity payment recorded in MySewa. Wallet
              debit equals bill amount plus charge minus cashback.
            </footer>
          </article>
        </div>
      )}
