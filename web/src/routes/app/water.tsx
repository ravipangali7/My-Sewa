...
import { ArrowLeft, Search, Droplets, ChevronRight, Check, RefreshCw } from "lucide-react";
...
/** Top-level drinking water companies. Counters load after a company is selected. */
const WATER_PROVIDERS: WaterProvider[] = [
  {
    id: "kukl",
    name: "KUKL",
    subtitle: "Kathmandu Upatyaka Khanepani Limited",
    color: "#0EA5E9",
  },
];
...
function WaterBillPayment() {
  ...
  const shellTitle =
    step === "provider" ? t("water.title") : t("water.paymentTitle", { provider: selectedProvider?.name ?? "" });

  const stepTitle = useMemo(() => {
    if (step === "provider") return t("water.stepProvider");
  ...
  return (
    <UserShell
      title={shellTitle}
      {...(step === "provider" ? { back: "/app" } : { onBack: goBack })}
      headerTrailing={
        <Button
          type="button"
          variant="ghost"
    ...
    >
      <div className="min-w-0 max-w-full space-y-5 overflow-x-clip">
        ...
        {step === "provider" ? (
          <section className="inset-group min-w-0 max-w-full overflow-hidden">
                ...
                {filteredProviders.map((provider) => (
                  <li key={provider.id}>
                    <button
                      ...
                      className="flex w-full items-center gap-3 px-4 py-3.5 text-left transition-colors hover:bg-muted/50 disabled:opacity-50"
                    >
                      <span
          ...
          <section className="inset-group min-w-0 max-w-full p-4">
            ...
            {step === "account" && selectedProvider ? (
              ...
              >
                <div className="flex items-center gap-3 rounded-xl border border-border bg-surface px-3 py-3 shadow-card">
                  ...
                  >
                    <RefreshCw className={cn("size-4", walletQuery.isFetching && "animate-spin")} />
                  </Button>
                </div>

                <div className="space-y-1.5">
                  <Label htmlFor="water_counter">{t("water.counter")}</Label>
                  {countersQuery.isLoading ? (
                    ...
                    <p className="py-3 text-sm text-muted-foreground">{t("water.noCounters")}</p>
                  ) : (
                    <Select
                ...
                <Button
                  ...
                  className="h-12 w-full rounded-xl text-[17px] font-semibold tracking-wide"
                >
