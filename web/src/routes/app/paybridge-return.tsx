...
type PayBridgeReturnSearch = {
  order?: string;
  deposit?: string;
  error?: string;
};

export const Route = createFileRoute("/app/paybridge-return")({
  validateSearch: (search: Record<string, unknown>): PayBridgeReturnSearch => ({
  ...
  head: () => ({
    meta: [
      { title: "PayBridgeNP Deposit — MySewa" },
      {
        name: "description",
        content: "Confirm your PayBridgeNP wallet deposit after returning from checkout.",
      },
    ],
  }),
  component: PayBridgeReturnPage,
});

function PayBridgeReturnPage() {
  const { t } = useI18n();
  ...
  const verifyQuery = useQuery({
    queryKey: ["paybridge-verify", order, depositId],
    enabled: Boolean(token) && (Boolean(order) || depositId > 0),
    queryFn: () =>
      apiClient.paybridgeVerify({
        ...(depositId > 0 ? { deposit_id: depositId, id: depositId } : {}),
        ...(order ? { order_id: order, order, purchase_order_identifier: order } : {}),
      }),
    retry: false,
  ...
  }, [verifyQuery.data?.data?.status, verifyQuery.data?.outcome, queryClient]);
