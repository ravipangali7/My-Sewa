...
const ACTIONS = [
  ...
  {
    to: "/app/electricity",
    labelKey: "home.electricity" as const satisfies MessageKey,
    icon: Zap,
    iconBg: "bg-[#EAB308]",
  },
  {
...
function WalletHome() {
  ...
  const manualLoadEnabled =
    settingsQuery.data?.config?.payment?.deposits_enabled !== false &&
    !accountPending &&
    !walletFrozen;
    ...
    <UserShell title="MySewa" hideHeader>
      ...
      <div className="relative flex min-w-0 max-w-full flex-col bg-[#F3F5F8] lg:min-h-0 lg:rounded-2xl lg:overflow-hidden">
        ...
        <div className="relative z-20 -mt-10 flex flex-col space-y-4 px-4 pb-4">
          ...
          {(accountPending || walletBlocked || walletFrozen) ? <AccountPendingBanner /> : null}
          ...
          <section className="grid shrink-0 grid-cols-3 gap-2.5">
            {ACTIONS.map((a) => {
              ...
              const remittance = a.to === "/app/remittance";
              const blocked =
                (accountPending && (outbound || remittance)) ||
                (walletFrozen && (outbound || remittance)) ||
              ...
              if (blocked) {
                return (
                  <button
                    ...
                    onClick={() =>
                      toast.error(
                        walletFrozen && (outbound || remittance) && !accountPending
